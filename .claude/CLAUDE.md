...
**Images.** A photo lives in the page bundle's own `images/` folder. Reference it with plain Markdown — `![alt text](images/photo.jpg)` — for a responsive image that fills the column, or use the `image` shortcode when you want a caption or a width cap: `{{< image src="images/photo.jpg" alt="..." caption="..." width="480" >}}`. Either way the build runs the image through its pipeline, which makes it responsive, fast, and privacy-safe — it even strips the GPS location cameras embed. `src` resolves against the page bundle first, then `assets/`, so the same path finds a page's own photo or a shared one. The mechanics (and the exact options) live in the `image` shortcode at `layouts/shortcodes/image.html` and the Markdown render hook at `layouts/_default/_markup/render-image.html`, both self-documented.
...
**Icons.** Icons can come from a range of places, so offer the owner the options. When something needs an icon, like a copy button or a social link, there's more than one way to get one: you can draw it, from your own idea or from the owner's description; the owner can supply their own file or sketch; or you can browse a set like [Material Symbols](https://fonts.google.com/icons) together, a nice way to see what's out there and give the owner ideas for the site they want to make. For brand and social marks like GitHub or Patreon, reach for [Simple Icons](https://simpleicons.org), a large CC0 set kept current as brands change. However the icon arrives, self-host it. A single-color icon can use `currentColor` like the logo, so it follows the surrounding text into light and dark; an icon whose own colors are the point keeps them.
