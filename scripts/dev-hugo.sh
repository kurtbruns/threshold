...
# --port (it has no PORT support), so the indirection lives here. Falls back to
# 1313 for a plain run.
#
# `exec` replaces this shell with hugo so Ctrl-C / stop signals reach hugo
# directly instead of an intermediate bash, avoiding an orphaned server that
